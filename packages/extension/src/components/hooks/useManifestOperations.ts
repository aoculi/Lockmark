...
import { useManifestMutation } from '@/components/hooks/useManifestMutation'
import { useManifestQuery } from '@/components/hooks/useManifestQuery'
import { decryptManifest } from '@/lib/api'
...
export function useManifestOperations() {
  ...
  const [storeState, setStoreState] = useState<ManifestStoreState>(() =>
  ...
  useEffect(() => {
    if (storeState.status === 'dirty' && mutateRef.current) {
      const timeoutId = setTimeout(() => {
        ...
        if (
          manifestStore.getState().status === 'dirty' &&
          mutateRef.current &&
          !mutation.isPending &&
          !isSavingRef.current
        ) {
