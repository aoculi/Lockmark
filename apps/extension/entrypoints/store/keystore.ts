...
export class KeyStoreManager {
  ...
  async isUnlocked(): Promise<boolean> {
    const [status, context] = await Promise.all([
      sendToBackground<{ ok: boolean; unlocked: boolean }>({
        type: 'keystore:isUnlocked'
      }),
      sendToBackground<{ ok: boolean; context: AadContext | null }>({
    ...
    return (
      ...
      Boolean(
        context?.context && context.context.userId && context.context.vaultId
      )
    )
  }
